				...
				<div class="row-span-1">
					<div class="grid content-center grid-cols-1">
						<div class="col-span-1 p-1">
							<InputText v-model="formData.justificativa" class="w-full lg:w-[16.6rem] h-[2.5rem]"
					...
					<div class="grid grid-rows-1 justify-center mt-8">
						<Button
							...
							@click="enviarFormularioDeCadastroDeJustificativa()" />
					</div>
				</div>
			</div>
		</form>
...
const enviarFormularioDeCadastroDeJustificativa = async () => {
	if (formData.justificativa === "") {
		...
		useCadastroService.cadastrarJustificativa(formData.value).then((response) => {
